  ...
  return (
    <div className="page">
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 32, fontWeight: 600, color: 'var(--plum)', lineHeight: 1.1 }}>{userName ? userName.split(' ')[0] : 'Welcome back'}</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
        <div className="card" style={{ margin: 0, cursor: 'pointer' }} onClick={() => setTab('goals')}>
          ...
          {latest ? (<>
            <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 28, fontWeight: 600, color: 'var(--plum)', lineHeight: 1 }}>{latest}<span style={{ fontSize: 13, fontWeight: 400, color: 'var(--muted)' }}>lbs</span></div>
        ...
        <div className="card" style={{ margin: 0, cursor: 'pointer' }} onClick={() => setTab('meals')}>
          ...
          {hasMeal ? (<>
            <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 28, fontWeight: 600, color: 'var(--plum)', lineHeight: 1 }}>{meal.days.length}<span style={{ fontSize: 13, fontWeight: 400, color: 'var(--muted)' }}> days</span></div>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 24, fontWeight: 600, color: 'var(--muted2)', lineHeight: 1 }}>No plan</div>
        ...
        <div className="card" style={{ margin: 0, cursor: 'pointer' }} onClick={() => setTab('pantry')}>
          ...
          <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 28, fontWeight: 600, color: 'var(--plum)', lineHeight: 1 }}>{pantry.length}<span style={{ fontSize: 13, fontWeight: 400, color: 'var(--muted)' }}> items</span></div>
        ...
        <div className="card" style={{ margin: 0 }}>
          ...
          <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 28, fontWeight: 600, color: 'var(--sage)', lineHeight: 1 }}>${estimatedSavings}</div>
          ...
          {Object.entries(todayMeals.meals || {}).map(([type, m]) => (
            <div key={type} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 15, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.name}</div>
      ...
      {!hasMeal && (
        <div style={{ background: 'var(--plumLL)', border: '1px solid var(--plum3)22', borderRadius: 14, padding: 16, textAlign: 'center' }}>
          <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 20, color: 'var(--plum)', marginBottom: 6 }}>Ready to plan your week?</div>
