...
export default function MealsTab({ pantry, goals, macros, meal, setMeal, setShop, setTab, notify, session }) {
  ...
  const generate = async () => {
    ...
    const sys = `You are a home cooking expert. Respond with ONLY raw JSON. No markdown, no backticks, no explanation.\nStructure: {"options":{"${exKey}":[{"name":"...","description":"...","uses_pantry":["..."],"calories":500,"protein":35,"carbs":55,"fat":15,"ingredients":["1 cup rice","2 chicken breasts"]}]}}\nRules: suggest FAMILIAR, PRACTICAL home meals that real families actually cook — like "Chicken Stir Fry", "Beef Tacos", "Pasta Bolognese", "Grilled Salmon", "Chicken Soup". Avoid exotic or unusual combinations. Names under 5 words, descriptions under 10 words, 4-8 common grocery ingredients per meal. IMPORTANT for ingredients: always use the most specific raw form — write "boneless skinless chicken breast" not "chicken breast", "raw shrimp" not "shrimp", "93% lean ground beef" not "ground beef", "fresh salmon fillet" not "salmon". This ensures correct grocery store matching.`
  ...
  const generateRecipe = async mealItem => {
    ...
    const sys = `You are a chef. Respond with ONLY raw JSON. No markdown. Structure: {"title":"...","servings":4,"prep_time":"15 min","cook_time":"30 min","ingredients":[{"amount":"1 cup","name":"brown rice"}],"steps":["Step 1: ..."]}`
